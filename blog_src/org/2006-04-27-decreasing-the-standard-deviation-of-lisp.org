#+TITLE: Decreasing The Standard Deviation Of Lisp
#+AUTHOR: Ben
#+DATE: 2006-04-27
#+TAGS: programming software-development reading meta

Recently, there was an (apparently cyclic) thread on comp.lang.lisp the
other day about how lisp sucks and why new developers aren't flocking to
bask in its glory.\\
\\
...
what I call a languages "standard deviation" to the current most popular
languages. The corollary is; a language that offers a low standard
deviation will have a higher appeal then a similar language with a
higher standard deviation. (Note: I'm doing some pseudo-statistics at
work; well, the statistics are real, but I'm not an real statistician,
hence the "pseudo" part)\\
\\
...
using lisp,
[[http://blade.nagaokaut.ac.jp/cgi-bin/scat.rb/ruby/ruby-talk/179642][why
didn't Matz just use lisp, or smalltalk?]]\\
\\
I think one of the big reasons (perhaps unconsciously), is that Matz
recognized the "standard deviation" between both lisp and smalltalk, and
set about designing a language that was closer to most programmers
...
so someone could just import that package, and be greeted with functions
that more closely mirrored their expectations.\\
\\
...
\\
Note, I think my arguments are implicitly supported by SteveY's
[[http://steve-yegge.blogspot.com/2006/04/lisp-is-not-acceptable-lisp.html][blog
...
had come to expect from most "mainstream" languages.\\
\\
